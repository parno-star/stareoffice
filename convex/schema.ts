...
export default defineSchema({
  ...
  // ---- Strategic Issues (Papan Isu Strategis) --------------------------------
  // Board-level issues / urgent priorities surfaced for director attention.
  // Managed (CRUD) by executives; directors monitor. Tenant-scoped.
  strategicIssues: defineTable({
    title: v.string(),
    description: v.optional(v.string()),
    // Person accountable (PIC).
    ownerId: v.id("users"),
    // Related unit/department name (optional; matches departments.name).
    department: v.optional(v.string()),
    // "low" | "medium" | "high" | "critical"
    urgency: v.string(),
    // "low" | "medium" | "high"
    impact: v.string(),
    // ISO 8601 UTC timestamp for the deadline (optional).
    dueDate: v.optional(v.string()),
    // "needs_decision" | "in_progress" | "monitoring" | "resolved"
    status: v.string(),
    // Optional link to a related objective.
  ...
  letterheads: defineTable({
    ...
    // Mode "kop utuh dari gambar": bila aktif, seluruh area kop diganti oleh satu
    // gambar yang diunggah (tanpa input teks, warna aksen, atau garis pembatas).
    useFullImage: v.optional(v.boolean()),
    fullImageStorageId: v.optional(v.id("_storage")),
    fullImageFileName: v.optional(v.string()),
    isDefault: v.boolean(),
    authorId: v.id("users"),
    organizationId: v.optional(v.id("organizations")),
  })
  ...
  // ---- BoD KPI Reports (data input via Excel upload) -------------------------
  // Stores KPI data submitted per division per period for all 4 categories:
  // Revenue, Production/Operations, HR, and Risk.
  bodKpiReports: defineTable({
    organizationId: v.id("organizations"),
    // "YYYY-MM" period key e.g. "2026-08"
    period: v.string(),
    // "revenue" | "production" | "hr" | "risk"
    category: v.string(),
    // Division name (one of 25 divisions)
    division: v.string(),
    // "jasa" | "manufaktur"
    divisionType: v.string(),
    // JSON blob of the actual KPI values (flexible per category)
    data: v.string(),
    ...
    .index("by_org_period_category", ["organizationId", "period", "category"])
    .index("by_org_period_division", ["organizationId", "period", "division"]),

  // ---- BoD Divisions Configuration -------------------------------------------
  // Stores the list of 25 divisions per organization with their metadata.
  bodDivisions: defineTable({
    organizationId: v.optional(v.id("organizations")),
    name: v.string(),
    // "jasa" | "manufaktur"
    type: v.string(),
  ...
  // ---- BoD Risk Register -----------------------------------------------------
  // Stores risk items for the risk matrix per division per period.
  bodRiskItems: defineTable({
    organizationId: v.id("organizations"),
    period: v.string(),
    division: v.string(),
    title: v.string(),
    description: v.optional(v.string()),
    // "strategic" | "operational" | "financial" | "compliance" | "hse"
    category: v.string(),
    // 1..5
    likelihood: v.number(),
    // 1..5
  ...
  // ---- BoD Audit Findings ---------------------------------------------------
  // Internal audit findings per division.
  bodAuditFindings: defineTable({
    organizationId: v.id("organizations"),
    period: v.string(),
    division: v.string(),
    auditTitle: v.string(),
  ...
  // ---- BoD HSE Incidents ----------------------------------------------------
  // HSE (Health, Safety, Environment) incidents per division.
  bodHseIncidents: defineTable({
    organizationId: v.id("organizations"),
    period: v.string(),
    division: v.string(),
    ...
    title: v.string(),
    description: v.optional(v.string()),
    // "near_miss" | "first_aid" | "medical_treatment" | "lost_time" | "fatality" | "property_damage"
    type: v.string(),
    // "low" | "medium" | "high" | "critical"
    severity: v.string(),
    location: v.optional(v.string()),
  ...
  // ---- BoD Tab Configuration --------------------------------------------------
  // Stores customizable labels and config per tab per organization.
  // `tabKey` = "revenue" | "production" | "hr" | "risk" | "audit" | "hse" | "exec"
  // `config` = JSON blob of label overrides and other settings
  bodTabConfig: defineTable({
    organizationId: v.optional(v.id("organizations")),
    tabKey: v.string(),
    config: v.string(), // JSON blob
    updatedBy: v.id("users"),
    updatedAt: v.string(),
  })
    .index("by_organization", ["organizationId"])
    .index("by_org_and_tab", ["organizationId", "tabKey"]),

  // Program vs Realisasi Achievement per divisi per KPI per periode
  bodDivisionKpi: defineTable({
    organizationId: v.id("organizations"),
    division: v.string(),
    divisionType: v.string(),
    period: v.string(),       // YYYY-MM
    kpiKey: v.string(),       // e.g. "total_revenue", "gross_profit"
    program: v.number(),      // target/program dari divisi
    realisasi: v.number(),    // realisasi aktual
    updatedAt: v.string(),
  })
    .index("by_org_period", ["organizationId", "period"])
    .index("by_org_period_div_kpi", ["organizationId", "period", "division", "kpiKey"]),

  // ---- Finance Reports (Laporan Kinerja Keuangan) --------------------------------
  // Diinput oleh Dept Keuangan, sync ke tab Finance dashboard BoD.
  financeReports: defineTable({
    organizationId: v.optional(v.id("organizations")),
    // Periode: "YYYY-MM" | "YYYY-Q1".."YYYY-Q4" | "YYYY-S1"|"YYYY-S2" | "YYYY"
    period: v.string(),
    // Tipe periode: "bulan" | "triwulan" | "semester" | "tahun"
    periodType: v.string(),
    // KPI values as JSON blob: { totalRevenue, totalBudget, grossProfit, ebitda, cashFlow, ar, ap }
    data: v.string(),
    submittedBy: v.id("users"),
    submittedAt: v.string(),
    updatedAt: v.string(),
    ...
    .index("by_org_period", ["organizationId", "period"])
    .index("by_org_submittedAt", ["organizationId", "submittedAt"]),
});
