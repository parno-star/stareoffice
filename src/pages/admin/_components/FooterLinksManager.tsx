...
export default function FooterLinksManager() {
  ...
  const safeAllLinks = allLinks ?? [];
  const safeLocalLinks = localLinks ?? [];

  // If table is empty, show seed button
  if (safeAllLinks.length === 0) {
    return (
      <Card>
        <CardHeader>
  ...
  const activeCount = safeLocalLinks.filter((l) => l.isActive).length;
  const totalCount = safeLocalLinks.length;
