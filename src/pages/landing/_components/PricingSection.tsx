...
export default function PricingSection() {
  ...
  const candidateList = Array.isArray(dbPlans) ? (dbPlans as DbPlanCandidate[]) : [];
  const hasValidDbPlans =
    candidateList.length > 0 &&
    candidateList.some((p) => p && typeof p === "object" && (p.slug || p.price || p.coreFeatures));

  const plans: PlanTier[] = hasValidDbPlans
    ? candidateList
        .filter((p): p is DbPlanCandidate & { slug: string } => Boolean(p && typeof p === "object" && p.slug))
        .map((p) => {
          ...
          return {
            ...
            limits: {
              employees: formatEmployees(p.maxEmployees ?? 0),
              storage: formatStorage(p.maxStorageMb ?? 0),
              support: SUPPORT_LABELS[p.supportLevel ?? ""] ?? p.supportLevel ?? "Email",
            },
