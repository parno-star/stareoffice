...
import { SIDEBAR_GROUPS, MENU_KEYS } from "@/convex/roles.ts";
...
function SidebarContent({ onNavigate, onCollapse }: { onNavigate?: () => void; onCollapse?: () => void }) {
  ...
  const allowedSet: Set<MenuKey> = new Set<MenuKey>(
    MENU_KEYS as ReadonlyArray<MenuKey>
  );
...
function AuthenticatedDashboard() {
  ...
  const { removeUser, isDemo } = useAuth();
  ...
  return (
    ...
    <div className="flex h-screen bg-background">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex items-center gap-3 border-b bg-card px-4 py-3 lg:px-6">
          ...
          {isDemo && (
            <div className="hidden sm:flex items-center gap-1.5 rounded-full border border-amber-500/30 bg-amber-500/10 px-2.5 py-1 text-xs font-medium text-amber-700 dark:text-amber-300">
              <Sparkles className="size-3.5 shrink-0" />
              <span>Mode Demo</span>
              <button
                type="button"
                onClick={() => setDemoModalOpen(true)}
                className="ml-1 underline font-semibold cursor-pointer hover:text-amber-900 dark:hover:text-amber-100"
              >
                Ganti Peran
              </button>
            </div>
    ...
    <SignInModal open={demoModalOpen} onOpenChange={setDemoModalOpen} />
    </>
    </NoOrganizationGuard>
    </TenantProvider>
  );
