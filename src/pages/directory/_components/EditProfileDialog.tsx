            ...
            >
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <FormField
                  ...
                  name="department"
                  render={({ field }) => {
                    const deptItems = (departments ?? []) as unknown as Array<{
                      department?: { _id?: string; name?: string };
                      name?: string;
                      _id?: string;
                    }>;
                    const deptNames = deptItems
                      .map((d) => d.department?.name ?? d.name ?? "")
                      .filter(Boolean);
                    const currentVal = field.value || "";
                      ...
                      <FormItem>
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            {deptItems.map((d) => {
                              const deptName = d.department?.name ?? d.name ?? "";
                              const deptId = d.department?._id ?? d._id ?? deptName;
                              if (!deptName) return null;
                              return (
                                <SelectItem key={deptId} value={deptName}>
                                  {deptName}
                                </SelectItem>
                              );
                            })}
