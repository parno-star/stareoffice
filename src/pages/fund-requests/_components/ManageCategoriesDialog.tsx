          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                {Object.entries(CATEGORY_CONFIG).map(([key, cfg], idx) => (
                  <div
                    key={`${key}-${idx}`}
                    className={cn(
              ...
              {isLoading ? (
                <div className="space-y-2">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <Skeleton key={`cat-skel-${i}`} className="h-14 w-full rounded-lg" />
                  ))}
                </div>
              ) : customs.length === 0 ? (
                <Empty>
                ...
                <div className="space-y-2">
                  {customs.map((row, idx) => {
                    ...
                    if (isEditing) {
                      return (
                        <div
                          key={row._id ? `${row._id}-${idx}` : `custom-edit-${idx}`}
                    ...
                    return (
                      <div
                        key={row._id ? `${row._id}-${idx}` : `custom-row-${idx}`}
                        className={cn(
