...
type LetterDetail = {
  ...
  >;
  letterhead: (Doc<"letterheads"> & { logoUrl: string | null; fullImageUrl: string | null }) | null;
...
export const getLetter = query({
  ...
  handler: async (ctx, args): Promise<LetterDetail | null> => {
    ...
    const letterheadWithLogo = letterhead
      ? {
          ...letterhead,
          logoUrl: letterhead.logoStorageId ? await ctx.storage.getUrl(letterhead.logoStorageId) : null,
          fullImageUrl: letterhead.fullImageStorageId ? await ctx.storage.getUrl(letterhead.fullImageStorageId) : null,
        }
      : null;
...
export const listLetterheads = query({
  args: {},
  handler: async (ctx): Promise<Array<Doc<"letterheads"> & { logoUrl: string | null; fullImageUrl: string | null }>> => {
    const me = await requireAuth(ctx);
    ...
    return await Promise.all(
      scoped.map(async (lh) => ({
        ...
        fullImageUrl: lh.fullImageStorageId ? await ctx.storage.getUrl(lh.fullImageStorageId) : null,
      }))
    );
  },
});
