...
const NAV_META: Record<MenuKey, { icon: LucideIcon; label: string; path: string }> = {
  ...
  bod_dashboard:   { icon: Landmark,        label: "Dashboard Direksi",    path: "/bod-dashboard" },
  strategic_issues:{ icon: TriangleAlert,    label: "Isu Strategis",        path: "/strategic-issues" },
  ...
  finance_laporan:   { icon: ClipboardList,     label: "Laporan Dashboard Direksi - Keuangan", path: "/laporan-keuangan" },
...
function SidebarContent({ onNavigate, onCollapse }: { onNavigate?: () => void; onCollapse?: () => void }) {
  ...
  const allowedSet: Set<MenuKey> =
    new Set<MenuKey>(
      isSuperAdminUser
        ? (MENU_KEYS as unknown as MenuKey[])
        : (allowedMenus && allowedMenus.length > 0 ? allowedMenus : (MENU_KEYS as unknown as MenuKey[]))
    );
  if (isSuperAdminUser) {
    allowedSet.add("strategic_issues");
  }
