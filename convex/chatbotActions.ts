...
export const sendMessage = action({
  ...
  handler: async (
    ...
    try {
      ...
      /* --- ORIGINAL PRODUCTION CODE (HERCULES AI GATEWAY) ---
      ...
      if (process.env.HERCULES_API_KEY) {
        try {
          ...
          console.warn("Hercules AI Gateway fallback:", err);
        }
      }

      if (!rawContent) {
        rawContent = `Halo **${context.user.name}**! Saya Asisten AI Star e-Office (Mode Lokal).\n\nAnda saat ini terhubung di organisasi **${context.user.department || "PT Star Digital Office"}** sebagai **${context.user.role}**.\n\nSistem manajemen perkantoran (Surat, HR, Absensi, Cuti, Proyek, dll.) berjalan aktif di lingkungan lokal Anda.`;
      }
