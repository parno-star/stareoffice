...
import { ArrowRight, Zap, Phone, LayoutDashboard } from "lucide-react";
import { useAuth } from "@/hooks/use-auth.ts";

export default function CTASection() {
  const navigate = useNavigate();
  const { isAuthenticated, setDemoLogin } = useAuth();

  const handleOpenApp = () => {
    if (!isAuthenticated) {
      setDemoLogin("super_admin");
    }
    navigate("/home");
  };
        ...
        >
          ...
          <div className="relative space-y-8">
            ...
            <div className="flex flex-wrap items-center justify-center gap-4">
              <Button
                size="lg"
                className="cursor-pointer gap-2 bg-white text-primary font-bold shadow-xl hover:bg-white/90"
              ...
              <SignInButton>
                <Button
                  size="lg"
                  className="cursor-pointer gap-2 border-2 border-white/40 bg-white/10 text-white hover:bg-white/20"
                >
                  Mode Demo / Login
                </Button>
              </SignInButton>
