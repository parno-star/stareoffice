...
export const getLetter = query({
  ...
  handler: async (ctx, args): Promise<LetterDetail | null> => {
    ...
    if (letter.organizationId && (organizationId === null || letter.organizationId !== organizationId)) {
      return null;
    }
...
export const getMyDispositionUnreadCount = query({
  ...
  handler: async (ctx): Promise<number> => {
    ...
    for (const d of pending) {
      ...
      const letter = await ctx.db.get(d.letterId);
      if (!letter) continue;
      if (organizationId === null || letter.organizationId !== organizationId) continue;
      count += 1;
    }
    return count;
  },
...
export const getMyUnfinishedLetterCount = query({
  ...
  handler: async (ctx): Promise<number> => {
    ...
    let count = 0;
    for (const l of mine) {
      if (organizationId !== null && l.organizationId !== organizationId) continue;
...
export const getIncomingLettersBadgeCount = query({
  ...
  handler: async (ctx): Promise<number> => {
    ...
    for (const l of [...asRecipient, ...asCc]) {
      ...
      if (orgId === null || l.organizationId !== orgId) continue;
