...
import {
  ...
  Columns3, Trash2, TableProperties,
} from "lucide-react";
...
function EditorToolbar({
  ...
  // Impor tabel dari Excel (.xlsx/.xls)
  ...
  // Impor tabel dari berkas Excel (.xlsx/.xls). Sheet pertama dibaca dengan
  // pustaka xlsx (berjalan di peramban) lalu dikonversi menjadi tabel HTML dan
  // DISISIPKAN pada posisi kursor (tidak mengganti seluruh isi surat). Nilai
  // hasil perhitungan Excel ikut terbawa karena xlsx membaca nilai sel, bukan
  // rumusnya.
  const handleXlsxFileSelected = async (
    e: React.ChangeEvent<HTMLInputElement>,
  ) => {
    const file = e.target.files?.[0];
    e.target.value = "";
    ...
    if (!isExcel) {
      toast.error("Hanya mendukung berkas Excel (.xlsx atau .xls).");
      return;
    }
    if (file.size > 10 * 1024 * 1024) {
      toast.error("Ukuran berkas maksimal 10 MB.");
    ...
    try {
      ...
      if (!firstSheetName) {
        toast.error("Berkas Excel tidak memiliki lembar (sheet).");
        return;
      }
      const sheet = workbook.Sheets[firstSheetName];
      // Baca sebagai matriks nilai (hasil perhitungan), sel kosong jadi "".
      const rows = XLSX.utils.sheet_to_json<Array<string | number | boolean | null>>(
        sheet,
        { header: 1, blankrows: false, defval: "" },
      );
      if (!rows.length) {
        toast.error("Lembar Excel kosong.");
        return;
      }
      // Samakan jumlah kolom di semua baris agar tabel rapi.
      ...
      const bodyRows = rows
        .map((row) => {
          const cells = Array.from({ length: colCount }, (_, i) =>
            `<td><p>${escapeHtml(row[i])}</p></td>`,
          ).join("");
          return `<tr>${cells}</tr>`;
        })
        .join("");
      const tableHtml = `<table>${bodyRows}</table><p></p>`;
      // Sisipkan pada posisi kursor tanpa mengganti isi surat lain.
      editor.chain().focus().insertContent(tableHtml).run();
      toast.success("Tabel dari Excel berhasil disisipkan.");
    } catch (err) {
      toast.error(
        err instanceof Error ? err.message : "Gagal mengimpor berkas Excel.",
      );
    } finally {
  ...
  return (
    <div className="sticky top-0 z-30 flex flex-wrap items-center gap-0.5 border-b bg-background p-2">
      ...
      {/* Edit tabel: tambah/hapus kolom & baris, hapus tabel (aktif saat di dalam tabel) */}
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button
            type="button"
            title="Edit Tabel (tambah/hapus kolom & baris)"
            disabled={!isInTable}
            className={cn(
              "flex size-7 cursor-pointer items-center justify-center rounded text-sm transition-colors hover:bg-accent",
              !isInTable && "cursor-not-allowed opacity-40",
            )}
          >
      ...
      {/* Impor tabel dari Excel: disisipkan pada posisi kursor */}
      <input
        ref={xlsxInputRef}
        type="file"
        accept=".xlsx,.xls,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel"
        className="hidden"
        onChange={(e) => { void handleXlsxFileSelected(e); }}
      />
      <ToolbarButton
        onClick={() => xlsxInputRef.current?.click()}
        disabled={isImportingXlsx}
        title="Sisipkan tabel dari Excel (.xlsx/.xls) pada posisi kursor"
      >
        {isImportingXlsx ? (
          <Loader2 className="size-3.5 animate-spin" />
        ) : (
          <span className="flex size-4 items-center justify-center rounded-[3px] bg-green-600 text-[10px] font-bold leading-none text-white">
            X
          </span>
        )}
      </ToolbarButton>
