...
function getSafeISOString(val: string | number | null | undefined): string {
  if (!val) return "";
...
export default function FundRecap() {
  ...
  const filtered = useMemo(() => {
    ...
    return allRequests.filter((r) => {
      const date = getSafeISOString(r.submittedAt) || getSafeISOString(r._creationTime);
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.slice(0, 50).map((r, idx) => {
                ...
                return (
                  <tr key={r._id ? `${r._id}-${idx}` : `row-${idx}`} className="border-b hover:bg-muted/20 last:border-0">
                    ...
                    <td className="px-4 py-2.5 text-muted-foreground text-xs">
                      {formatDate(getSafeISOString(r.submittedAt) || getSafeISOString(r._creationTime))}
                    </td>
                  </tr>
                );
              })}
